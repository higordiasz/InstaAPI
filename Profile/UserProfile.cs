...
namespace InstaAPI.Profile.UserProfile
{
    public static class UserProfileClass
    {
        ...
        /// <summary>
        /// Old method to find profile info by username
        /// </summary>
        /// <param name="insta">Instagram instance</param>
        /// <param name="username">Username</param>
        /// <returns>This method is obsolete, use 'GetUserProfileFromUsername'</returns>
        [Obsolete]
        public static async Task<IReturn> GetUserProfileByUsernameOld(this Instagram insta, string username, int attemps = 0)
        {
            IReturn ret = new(0, "Default");
            try
            {
                ...
                if (preload)
                {
                    ...
                    insta.HeaderFollow();
                    HttpRequestMessage req = new(HttpMethod.Get, $"https://www.instagram.com/{username}/?__a=1");
                    HttpResponseMessage res = await insta.Client.SendAsync(req);
                    string resContent = await res.Content.ReadAsStringAsync();
                    if (res.IsSuccessStatusCode)
                    {
                    ...
                    else
                    {
                        ...
                        if (resContent.IndexOf("Go back to Instagram.") > -1 && attemps < 3)
                        {
                            await Task.Delay(1246);
                            int i = attemps + 1;
                            return await insta.GetUserProfileByUsernameOld(username, i);
                        }
                        string title = WebHelperClass.GetTitle(resContent);
                        if (title.IndexOf("Instagram") > -1)
                        {
            ...
            {
                ret.Err = err;
                insta.ErrWrite(err, "GetUserProfileByUsernameOld");
                ret.Status = -1;
                ret.Response = "An unexpected error has occurred";
            }
            return ret;
